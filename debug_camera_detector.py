...
class DebugCameraDetector:
    def __init__(self):
        ...
        print("Debug Camera Face Mask Detection Started!")
        print(f"Camera opened successfully at index {self.camera_index} with backend {self.backend}!")
    ...
    def add_info_panel(self, frame, face_count, mask_count, status="Running", frame_info=""):
        ...
        cv2.putText(frame, "Debug Camera Face Mask Detection", (20, 35), 
        ...
        cv2.putText(frame, f"Masks Detected: {mask_count}", (20, 110), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
        cv2.putText(frame, f"Camera: Index {self.camera_index}, Backend {self.backend}", (20, 135), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
        cv2.putText(frame, f"Frame Info: {frame_info}", (20, 155), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
    
    def run_detection(self):
        """Main detection loop with debugging"""
        print("Starting Debug Camera Face Mask Detection...")
        print(f"Using camera index {self.camera_index} with backend {self.backend}")
        ...
        while True:
            ...
            if not ret or frame is None:
                ...
                if failed_frames > 10:
                    print("Too many failed frames. Stopping.")
                    break
                
                # Show error frame
                error_frame = np.zeros((480, 640, 3), dtype=np.uint8)
                cv2.putText(error_frame, f"Camera Error - Frame {frame_count}", (150, 240), 
                           cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
                cv2.imshow('Debug Camera Face Mask Detection', error_frame)
                cv2.waitKey(1000)  # Wait 1 second
                continue
            
            failed_frames = 0  # Reset on successful capture
            
            # Get frame info
            frame_height, frame_width = frame.shape[:2]
            frame_info = f"{frame_width}x{frame_height} - Frame {frame_count}"
            ...
            self.add_info_panel(frame, face_count, mask_count, "Running", frame_info)
            
            # Display frame
            cv2.imshow('Debug Camera Face Mask Detection', frame)
